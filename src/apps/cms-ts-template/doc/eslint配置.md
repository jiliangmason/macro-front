...
模板中的配置主要使用 Facebook 开发的`eslint-config-react-app`，其中包括了所有正常开发中需要的配置，在此基础上，引入了`eslint-config-airbnb`，使用 Airbnb 配置的代码格式规则，然后增加了 TypeScript 检查，最终引入`prettier`，如果 ESLint 配置和 prettier 冲突，则优先使用 prettier 规则。
